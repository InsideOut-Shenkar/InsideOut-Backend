...
class Predictor(ABC):
    ...
    def train_model(self):
        if os.path.isfile(os.path.join('tmp', os.path.basename(self.path))) and self.load_model(self.path) is not None:
            return
    ...
    def predict(self, row):
        ...
        """
        if self.path is not None and os.path.isfile(os.path.join('tmp', os.path.basename(self.path))):
